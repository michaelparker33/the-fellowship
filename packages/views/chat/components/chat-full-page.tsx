...
export function ChatFullPage() {
  ...
  useChatStreaming();
  const { handleSend, handleStop } = useChatSend(activeAgent);
          ...
          {group.items.map((session) => {
            ...
            return (
              <button
                ...
                className={`group flex w-full items-center gap-2 px-3 py-2 text-left transition-colors rounded-md mx-1 ${
                  isActive
              ...
              >
                ...
                {session.status === "archived" && (
                  <Archive className="size-3 shrink-0 opacity-40" />
                )}
